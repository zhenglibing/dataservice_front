...
@Component({
  ...
  templateUrl: './in-stock-chart.page.html',
  styleUrls: ['./in-stock-chart.page.scss'],
})
export class InStockChartPage implements OnInit {
  ...
  weight :any = '';
  convertStdNum :any = '';
  constructor(private router: Router, private activatedRoute: ActivatedRoute,private env: EnvService, private echartservice: EchartService, private service: InStockService) { }
  ...
  public loadData() {
    ...
    this.activatedRoute.queryParams.subscribe(param => {
      ...
      //渲染图表
      this.service.getBusInStockChart(this.startDate,
                                          this.endDate,
                                          this.materCode,
                                          this.offset,
        ...
        if (res && res.state && res.state === '0') {
          ...
          if (res.result && res.result.length > 0) {
            ...
            // 设置标题文本
            this.options.title.subtext = '胶料入库趋势图';
          ...
          console.warn('请求后台接口[getBusInStockChart]异常!');
        }
      }).catch(err => {
        console.error(err);
      });
      ...
      //加载列表数据
      this.service.getBusInStockChartList(
                        param['startDate'],
                        param['endDate'],
                        param['materCode']
                      ).then((res: any) => {
        ...
        console.warn('请求后台接口[getBusInStockChartList]异常!');
        }
        }).catch(err => {
        console.error(err);
        });
